...
func ParseTraceback(r io.Reader) (*Traceback, error) {
	...
	for s.Scan() {
		line := s.Text()
		if trace.Reason == "" {
			if reason, ok := parseReason(s, line); ok {
				trace.Reason = reason
				continue
			}
			// no reason???
		...
		if stack, ok := parseStack(s, line); ok {
...
var reasonRE = regexp.MustCompile(`(panic|fatal error|SIG\w+):`)

func parseReason(s *bufio.Scanner, line string) (reason string, ok bool) {
